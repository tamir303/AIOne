...
4. **Decide whether GitHub's auto-merge is available**: `gh repo view --json autoMergeAllowed`. If it's on, you (the orchestrator) can use `gh pr merge --auto`, but it only gates the merge on CI if a status check is actually marked *required* on `main` — a workflow existing isn't enough by itself. See [Merge](github-protocol.md#merge) for how to verify this. If auto-merge is off, or no check is required, autonomous merge means immediate merge on approval with no automated check gating it, which is worth flagging to the user before you start.
